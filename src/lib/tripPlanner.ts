...
const clearlyHigherTierHotelName=/\b(?:luxury|premium|palace|legend|ritz(?:-carlton)?|four seasons|mandarin oriental|rosewood|aman(?:oi|puri|sara)?|capella|park hyatt|st\.? regis|waldorf|six senses|raffles|belmond|peninsula|intercontinental|sofitel|jw marriott|vinpearl resort|emeralda resort|apricot hotel)\b/i;
...
const higherTierHotelCopy=/\b(?:luxury|premium|five[- ]star|5[- ]star|top[- ]tier|palatial|ultra[- ]luxury)\b/i;
...
export function assessTripSuggestionQuality(value:unknown,profile:TripPlannerRequest,researchSources:TravellerResearchSource[]=[]):string[]{
  ...
  if(rawHotelStays.length){
    ...
    if(hotels.some(stay=>stay.options.some(option=>!hotelOptionMatchesBudget(option,profile.budget))))issues.push(`Remove hotels whose name, brand or description clearly signals a higher tier than the requested ${hotelStandardForBudget(profile.budget)} standard.`);
