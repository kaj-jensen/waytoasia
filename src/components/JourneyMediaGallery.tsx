...
export default function JourneyMediaGallery({items,name,labels=defaults}:{items:JourneyMedia[];name:string;labels?:Labels}){
 ...
 function close(){dialog.current?.close();setIsOpen(false);document.body.style.overflow='';opener.current?.focus();}
 function open(){setIsOpen(true);dialog.current?.showModal();document.body.style.overflow='hidden';}
 ...
 return <div className="journey-media" aria-label={`${name} · ${labels.gallery}`}><div className="media-main"><img src={photoSize(item.src,1200)} srcSet={photoSrcSet(item.src)} sizes="(max-width: 760px) 100vw, (max-width: 1200px) 58vw, 720px" alt={item.alt} fetchPriority="high"/><button className="media-open" type="button" ref={opener} onClick={open} aria-label={labels.view}>⛶ {labels.view}</button><button className="media-prev" type="button" onClick={()=>select(index-1)} aria-label={labels.previous}>‹</button><button className="media-next" type="button" onClick={()=>select(index+1)} aria-label={labels.next}>›</button><div className="media-caption" aria-live="polite"><span>{item.caption}</span><span>{index+1} / {items.length}</span></div></div>{thumbs()}<dialog className="media-dialog" ref={dialog} aria-label={`${name} · ${labels.gallery}`} onCancel={e=>{e.preventDefault();close();}} onKeyDown={e=>{if(e.key==='ArrowRight'){e.preventDefault();select(index+1);}if(e.key==='ArrowLeft'){e.preventDefault();select(index-1);}}}><div className="media-dialog-top"><span>{name} · {labels.gallery}</span><button type="button" onClick={close} autoFocus aria-label={labels.close}>{labels.close} ×</button></div><div className="media-enlarged">{isOpen&&<img src={item.src} alt={item.alt}/>}<button type="button" className="media-prev" onClick={()=>select(index-1)} aria-label={labels.previous}>‹</button><button type="button" className="media-next" onClick={()=>select(index+1)} aria-label={labels.next}>›</button></div><div className="media-dialog-caption" aria-live="polite">{item.caption}<span>{index+1} / {items.length}</span></div>{thumbs(true)}</dialog></div>;
}
