...
export const onRequestPost=async({request,env}:PagesContext):Promise<Response>=>{
  ...
  const text=[`Traveller: ${name}`,`Email: ${email}`,phone?`Phone: ${phone}`:'',`Language: ${locale}`,message?`Traveller note: ${message}`:'','',`AI itinerary: ${title}`,duration,summary,'',...route].filter(Boolean).join('\n');
  const sent=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({from:'Way to Asia <journeys@waytoasia.com>',to:[env.LEAD_TO_EMAIL],reply_to:email,subject:`AI trip enquiry: ${title} · ${name}`,text}),signal:AbortSignal.timeout(12000)});
