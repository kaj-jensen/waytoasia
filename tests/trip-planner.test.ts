...
test('builds long researched itineraries in parallel day batches',async()=>{
  ...
  const source='https://www.tripadvisor.com/Thailand/example?m=19905';
  const sourceTwo='https://www.reddit.com/r/ThailandTourism/example';
  const hotel=(id:string,name:string)=>({id,name,area:'Central and practical for daily sightseeing.',standard:'Comfort, with dependable facilities and service.',whyFit:'A well-located base for this balanced route.',roomGuidance:'Request a larger twin or double room and verify its exact size.',reviewSignal:'Traveller feedback highlights location and comfort, with some variation by room category.',sourceUrls:['https://tripadvisor.com/Thailand/example']});
  ...
  const days=(start:number,end:number)=>Array.from({length:end-start+1},(_,offset)=>{const day=start+offset;const place=day<=3?'Bangkok':day<=6?'Ayutthaya':day<=9?'Sukhothai':day<=13?'Chiang Mai':'Chiang Rai';return {day,place,theme:`History and nature day ${day}`,options:[{id:`d${day}-history`,name:`Named heritage experience ${day}`,type:'heritage',description:'Explore a named historic district or monument with enough context to make the day specific and useful.',whyFit:'Matches the requested history focus at a balanced pace.',interestTags:['history'],sourceUrls:['https://invented.example/activity']},{id:`d${day}-nature`,name:`Named nature experience ${day}`,type:'nature',description:'Spend time in a named park, river landscape or mountain setting connected to this base.',whyFit:'Adds the requested nature component without changing hotels.',interestTags:['nature'],sourceUrls:[sourceTwo]}]}});
  const originalFetch=globalThis.fetch;let modelCalls=0;const tavilyQueries:string[]=[];
  globalThis.fetch=async(url,init)=>{
    if(String(url).includes('api.tavily.com')){tavilyQueries.push((JSON.parse(String(init?.body||'{}')) as {query?:string}).query??'');return new Response(JSON.stringify({results:[{title:'Thailand traveller research',url:source,content:'Current traveller and hotel review context with a public price.'},{title:'Thailand forum research',url:sourceTwo,content:'Current route and destination discussion with an excursion price.'}]}),{status:200,headers:{'Content-Type':'application/json'}})}
  ...
  assert.ok(tavilyQueries.some(query=>query.includes('Bangkok Comfort One')&&query.includes('publicly listed room price')));
  assert.ok(tavilyQueries.some(query=>query.includes('Named heritage experience 1')&&query.includes('publicly listed excursion')));
...
test('returns a valid itinerary when an editorial advisory remains after repair',async()=>{
  const request=new Request('https://waytoasia.com/api/trip-suggestion',{method:'POST',headers:{'Content-Type':'application/json','Origin':'https://waytoasia.com'},body:JSON.stringify({locale:'en',destinationIdeas:'Japan',durationDays:7,adults:2,children:0,budget:'comfort',pace:'balanced',interests:['food','history']})});
  const draft={title:'Japan food and history',summary:'Here is a balanced 7-day Japan route pairing Tokyo neighbourhood food and history with Kyoto temple districts and a focused Nara visit, moving Tokyo → Kyoto/Nara.',recommendedDuration:'7 days',route:[{days:'Days 1–6',place:'Japan: Tokyo',plan:'Use Tokyo as a comfortable first base for historic neighbourhoods, market food and contrasting districts without changing hotels every night.',highlights:['Tsukiji Outer Market','Yanaka'],onwardTravel:'Continue by train to Kyoto.'},{days:'Day 7',place:'Japan: Kyoto & Nara',plan:'Use Kyoto as the overnight base and choose a focused Nara visit before returning for a final Kyoto evening.',highlights:['Nara Park','Gion'],onwardTravel:''}],fitReasons:['Balances food and history.','Keeps Tokyo as a stable base.'],practicalNotes:['Compare current individual Tokyo–Kyoto rail fares before deciding whether any pass offers value.','Spring particularly suits the neighbourhood walks and temple gardens on this route.','For a slower pace, keep the final day entirely in Kyoto and omit Nara.'],travellerInsights:[],matchedJourneySlugs:[],closing:'Would you like to keep Nara or make the final day slower in Kyoto?'};
