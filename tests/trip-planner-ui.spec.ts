...
test('trip planner creates a clearly unbooked itinerary suggestion',async({page})=>{
  ...
  await expect(page.getByText(/per adult|per person/i)).toHaveCount(0);
  await page.getByText('What this estimate covers').click();
  await expect(page.getByText(/11 hotel nights.*3 selected stays/)).toBeVisible();
  await expect(page.getByText('booking.com, getyourguide.com')).toBeVisible();
  await expect(page.getByText(/International flights are not included/)).toBeVisible();
